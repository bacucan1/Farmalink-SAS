  ...
  async function eliminar(u: Usuario) {
    try {
      ...
      if (res.ok) {
        ...
        await loadUsuarios();
        addToast('🗑️ Usuario eliminado', 'success');
      } else {
            ...
            {filtrados.length === 0 ? (
              ...
              <tr key={u.id}>
                ...
                <td className="td-actions">
                  <button className="btn-edit" onClick={() => abrirEditar(u)}>✏️ Editar</button>
                  <button className="btn-delete" onClick={() => setConfirmDelete(u)}>🗑️ Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
