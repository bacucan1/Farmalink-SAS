...
router.get('/', async (_req, res) => {
  try {
    const pool = Database.getInstance().getPool();
    const result = await pool.query(
      'SELECT id, name, email, role, created_at FROM public.usuarios ORDER BY created_at DESC'
    );
...
router.get('/:id', async (req, res) => {
  try {
    const pool = Database.getInstance().getPool();
    const result = await pool.query(
      'SELECT id, name, email, role, created_at FROM public.usuarios WHERE id = $1',
      [req.params.id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Usuario no encontrado' });
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const returningCols: string[] = ['id', 'name', 'email', 'role', 'created_at'];
    let result;
    try {
