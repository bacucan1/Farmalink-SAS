...
import { requireAuth, requireAdmin } from '../shared/authMiddleware.js';
...
router.get('/me', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT id, name, email, role, telefono, created_at FROM usuarios WHERE id = $1',
      [usuarioActual.id]
    );
...
// Nota: el gateway ya verificó que el solicitante es admin
