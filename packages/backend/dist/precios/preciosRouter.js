...
router.get('/comparar/:medicamentoId', requireAuth, async (req, res) => {
    ...
    `, [medId]);
        ...
        const precioItems = preciosResult.rows.map((p) => ({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const quienCambio = req.body.quien_cambio || req.body.userEmail || req.headers['x-user-email'] || 'sistema';
        ...
        const result = await pool.query(`UPDATE precios SET precio = $1, fecha = NOW() WHERE id = $2 RETURNING *`, [req.body.precio, id]);
        const medResult = await pool.query('SELECT name, lab FROM medicamentos WHERE id = $1', [current.medicamento_id]);
        const farResult = await pool.query('SELECT name, address FROM farmacias WHERE id = $1', [current.farmacia_id]);
        res.json({
            success: true,
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        values.push(id);
        const result = await pool.query(`UPDATE precios SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
        res.json({
            success: true,
            message: 'Precio actualizado correctamente',
            data: result.rows[0],
...
router.get('/historial/:medicamentoId', async (req, res) => {
    try {
        ...
        const historialResult = await pool.query(`SELECT
         h.id,
         h.precio_id,
         h.farmacia_id,
       ...
       ORDER BY h.fecha_cambio ASC`, [medicamentoId]);
        // Obtener el precio actual de cada farmacia para agregarlo como último punto
        const preciosActualesResult = await pool.query(`SELECT
         p.id,
         p.farmacia_id,
         f.name    AS farmacia_nombre,
         f.address AS farmacia_direccion,
       ...
       WHERE p.medicamento_id = $1`, [medicamentoId]);
...
//# sourceMappingURL=preciosRouter.js.map