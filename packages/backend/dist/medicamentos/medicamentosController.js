...
import { validateCreateMedicamento, validateUpdateMedicamento, } from './MedicamentoDTO.js';
export async function getAll(req, res) {
    ...
    `;
        ...
        // DISTINCT ON (m.id) toma la primera fila según ORDER BY: usamos la última fecha de precio.
        query += ' ORDER BY m.id, p.fecha DESC NULLS LAST';
...
export async function getById(req, res) {
    try {
        const { id } = req.params;
        const pool = Database.getInstance().getPool();
        const result = await pool.query(`SELECT DISTINCT ON (m.id)
              m.*, c.nombre as categoria_nombre, c.orden as categoria_orden,
              f.id as "farmaciaId", f.name as "farmaciaNombre",
              p.precio as precio
       FROM medicamentos m
       ...
       LEFT JOIN farmacias f ON p.farmacia_id = f.id
       WHERE m.id = $1
       ORDER BY m.id, p.fecha DESC NULLS LAST`, [id]);
...
export async function create(req, res) {
    try {
        ...
        const pool = Database.getInstance().getPool();
        const result = await pool.query(`INSERT INTO medicamentos (name, lab, active, description, categoria_id)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [
            body.name.trim(),
            body.lab.trim(),
...
export async function update(req, res) {
    try {
        ...
        params.push(id);
        const result = await pool.query(`UPDATE medicamentos SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, params);
        if (result.rows.length === 0) {
            res.status(404).json({ success: false, message: 'Medicamento no encontrado' });
            return;
        }
...
//# sourceMappingURL=medicamentosController.js.map