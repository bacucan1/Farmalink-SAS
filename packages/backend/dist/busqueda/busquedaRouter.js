...
router.get('/', async (req, res) => {
    try {
        ...
        const orden = (req.query.orden || 'relevancia').toLowerCase();
        const disponible = req.query.disponible;
        const precioMin = req.query.precioMin ? parseInt(req.query.precioMin) : null;
        const precioMax = req.query.precioMax ? parseInt(req.query.precioMax) : null;
        ...
        function buildExtraFilters(startPi) {
            const extraParams = [];
            let extraSql = '';
            let pi = startPi;
            if (categoria) {
        ...
        // ── SELECT base ────────────────────────────────────────────────────────
        function buildSelect(scoreExpr) {
            return `
        SELECT
      ...
      `;
        ...
        if (q) {
            ...
            if (totalP1 > 0) {
                ...
                const p2Params = [q]; // $1 para word_similarity
                let p2Pi = 2;
...
//# sourceMappingURL=busquedaRouter.js.map